...
function getGraficoFiltrado(filters, idAeroporto) {
...
Subconsulta AS (
    ...
    SELECT 'Qualidade das Instalações do Estacionamento' AS Coluna, E.Qualidade_Instalacoes_Estacionamento AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Estacionamento E ON T.Pesquisa_ID = E.Pesquisa_ID
    WHERE E.Qualidade_Instalacoes_Estacionamento IS NOT NULL
    ${filterConditions}
    
    UNION ALL
    
    SELECT 'Facilidade de Encontrar Vagas' AS Coluna, E.Facilidade_Encontrar_Vagas AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Estacionamento E ON T.Pesquisa_ID = E.Pesquisa_ID
    WHERE E.Facilidade_Encontrar_Vagas IS NOT NULL
    ${filterConditions}
    
    UNION ALL
    
    SELECT 'Facilidade de Acesso ao Terminal' AS Coluna, E.Facilidade_Acesso_Terminal AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Estacionamento E ON T.Pesquisa_ID = E.Pesquisa_ID
    WHERE E.Facilidade_Acesso_Terminal IS NOT NULL
    ${filterConditions}
    
    UNION ALL
    
    SELECT 'Relação Preço/Qualidade' AS Coluna, E.Relacao_Preco_Qualidade AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Estacionamento E ON T.Pesquisa_ID = E.Pesquisa_ID
    WHERE E.Relacao_Preco_Qualidade IS NOT NULL
    ${filterConditions}
    
    UNION ALL
    
    SELECT 'Tempo de Caminhada do Estacionamento aos Terminais' AS Coluna, E.Tempo_Caminhada_Estacionamento_Terminais AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Estacionamento E ON T.Pesquisa_ID = E.Pesquisa_ID
    WHERE E.Tempo_Caminhada_Estacionamento_Terminais IS NOT NULL
    ${filterConditions}
    
    UNION ALL
    
    SELECT 'Tempo de Espera do Ônibus de Deslocamento' AS Coluna, E.Tempo_Espera_Onibus_Deslocamento_Estacionamento_Terminais AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Estacionamento E ON T.Pesquisa_ID = E.Pesquisa_ID
    WHERE E.Tempo_Espera_Onibus_Deslocamento_Estacionamento_Terminais IS NOT NULL
    ${filterConditions}
    
    UNION ALL
    
    SELECT 'Quantidade de Estabelecimentos de Alimentação' AS Coluna, E.Quantidade_Estabelecimentos_Alimentacao AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Estabelecimentos E ON T.Pesquisa_ID = E.Pesquisa_ID
    WHERE E.Quantidade_Estabelecimentos_Alimentacao IS NOT NULL
    ${filterConditions}
    
    UNION ALL
    
    SELECT 'Qualidade e Variedade das Opções de Alimentação' AS Coluna, E.Qualidade_Variedade_Opcoes_Alimentacao AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Estabelecimentos E ON T.Pesquisa_ID = E.Pesquisa_ID
    ...
    SELECT 'Relação Preço/Qualidade da Alimentação' AS Coluna, E.Relacao_Preco_Qualidade_Alimentacao AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Estabelecimentos E ON T.Pesquisa_ID = E.Pesquisa_ID
    ...
    SELECT 'Quantidade de Estabelecimentos Comerciais' AS Coluna, E.Quantidade_Estabelecimentos_Comerciais AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Estabelecimentos E ON T.Pesquisa_ID = E.Pesquisa_ID
    ...
    SELECT 'Qualidade e Variedade das Opções Comerciais' AS Coluna, E.Qualidade_Variedade_Opcoes_Comerciais AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Estabelecimentos E ON T.Pesquisa_ID = E.Pesquisa_ID
    ...
    SELECT 'Relação Preço/Qualidade dos Comerciais' AS Coluna, E.Relacao_Preco_Qualidade_Comerciais AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Estabelecimentos E ON T.Pesquisa_ID = E.Pesquisa_ID
    ...
    SELECT 'Tempo de Espera na Fila (Inspeção de Segurança)' AS Coluna, I.Tempo_Espera_Fila AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Inspecao_Seguranca I ON T.Pesquisa_ID = I.Pesquisa_ID
    WHERE I.Tempo_Espera_Fila IS NOT NULL
    ${filterConditions}
    ...
    SELECT 'Atendimento dos Funcionários (Inspeção de Segurança)' AS Coluna, I.Atendimento_Funcionarios AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Inspecao_Seguranca I ON T.Pesquisa_ID = I.Pesquisa_ID
    WHERE I.Atendimento_Funcionarios IS NOT NULL
    ${filterConditions}
    ...
    SELECT 'Processo de Restituição de Bagagens' AS Coluna, RB.Processo_Restituicao_Bagagens AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    ...
    SELECT 'Facilidade de Identificação da Esteira' AS Coluna, RB.Facilidade_Identificacao_Esteira AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Restituicao_Bagagens RB ON T.Pesquisa_ID = RB.Pesquisa_ID
    ...
    SELECT 'Tempo de Restituição' AS Coluna, RB.Tempo_Restituicao AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Restituicao_Bagagens RB ON T.Pesquisa_ID = RB.Pesquisa_ID
    ...
    SELECT 'Integridade da Bagagem' AS Coluna, RB.Integridade_Bagagem AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Restituicao_Bagagens RB ON T.Pesquisa_ID = RB.Pesquisa_ID
    ...
    SELECT 'Atendimento da Companhia Aérea' AS Coluna, RB.Atendimento_Cia_Aerea AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Restituicao_Bagagens RB ON T.Pesquisa_ID = RB.Pesquisa_ID
    ...
    SELECT 'Quantidade de Banheiros' AS Coluna, S.Quantidade_Banheiros AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Sanitarios S ON T.Pesquisa_ID = S.Pesquisa_ID
    ...
    SELECT 'Manutenção Geral dos Sanitários' AS Coluna, S.Manutencao_Geral_Sanitarios AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Sanitarios S ON T.Pesquisa_ID = S.Pesquisa_ID
    ...
    SELECT 'Limpeza Geral do Aeroporto' AS Coluna, S.Limpeza_Geral_Aeroporto AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Sanitarios S ON T.Pesquisa_ID = S.Pesquisa_ID
    ...
    SELECT 'Controle Migratório' AS Coluna, M.Controle_Migratorio AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Controle_Migratorio_Aduaneiro M ON T.Pesquisa_ID = M.Pesquisa_ID
    WHERE M.Controle_Migratorio IS NOT NULL
    ${filterConditions}
    
    UNION ALL
    
    SELECT 'Atendimento dos Funcionários (Controle Migratório)' AS Coluna, M.Atendimento_Funcionarios AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Controle_Migratorio_Aduaneiro M ON T.Pesquisa_ID = M.Pesquisa_ID
    WHERE M.Atendimento_Funcionarios IS NOT NULL
    ${filterConditions}
    
    UNION ALL
    
    SELECT 'Controle Aduaneiro' AS Coluna, M.Controle_Aduaneiro AS Valor, T.Trimestre
    FROM CTE_Trimestre T
    LEFT JOIN Controle_Migratorio_Aduaneiro M ON T.Pesquisa_ID = M.Pesquisa_ID
    WHERE M.Controle_Aduaneiro IS NOT NULL
    ${filterConditions}
),
Subconsulta AS (
    SELECT 'Satisfacao_Geral' AS Coluna, P.Satisfacao_Geral AS Valor, T.Trimestre
...
SELECT Coluna, ROUND(AVG(Valor), 2) AS Media, CONCAT(SUBSTRING(Trimestre, 1, 3), ' Tri/', SUBSTRING(Ano, 3, 2)) AS Trimestre_Ano
FROM Subconsulta
GROUP BY Coluna, Trimestre_Ano
ORDER BY Trimestre_Ano, Coluna;
    `;

    console.log("Erro no Grafico Filtrado");
    console.log("Condições de filtro:", filters);
    ...
    return database.executar(query)
        ...
        .catch(error => {
            console.error('Erro ao executar a query filtrada:', error);
            throw error;
        });
